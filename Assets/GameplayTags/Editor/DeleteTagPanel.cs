...
namespace BandoWare.GameplayTags.Editor
{
   internal class DeleteTagPanel
   {
         ...
         if (GUILayout.Button("Delete"))
         {
            ...
            if (!HasError)
            {
               try
               {
                  ...
                  {
                     foreach (FileGameplayTagSource source in m_SourceFileOptions)
                     {
                        if (source != null)
                  ...
                  {
                     FileGameplayTagSource source = GetSelectedFileTagSource();
                     source.DeleteTag(m_TagToDelete.Name);
                  }
      ...
      private FileGameplayTagSource GetSelectedFileTagSource()
      {
         if (IsAllSourcesSelected())
            throw new InvalidOperationException("All Sources selected. No single source available.");

         FileGameplayTagSource source = (FileGameplayTagSource)m_SourceFileOptions[m_SelectedSourceFileIndex];
         return source;
      }
